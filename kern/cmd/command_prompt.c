...
//---------------------------------task 1
int is_subseq(int i, char *input) {
	const char *ptr1 = input;
...
void delete_List() {
	struct Command *cur = foundCommands.lh_first;
	struct Command *nxt;

	while (cur != NULL) {

		nxt = cur->prev_next_info.le_next;
		cur->prev_next_info.le_next = NULL;
		cur->prev_next_info.le_prev = NULL;
		cur = nxt;
	}
	LIST_INIT(&foundCommands);

}
void found_list(struct Command * command) {

	struct Command * cmd = command;
	foundCommands.___ptr_next = command;
	if (foundCommands.size
			== 0&& foundCommands.lh_first==NULL && foundCommands.lh_last==NULL) {
		//cprintf("first\n");
		foundCommands.lh_first = foundCommands.___ptr_next;
		foundCommands.lh_last = foundCommands.___ptr_next;

		foundCommands.size += 1;
	} else {
		//cprintf("next..\n");
		/* foundCommands.lh_last->prev_next_info.le_next=tmp;
		 foundCommands.lh_last=tmp;*/
		foundCommands.lh_last->prev_next_info.le_next =
				foundCommands.___ptr_next;
		foundCommands.lh_last = foundCommands.___ptr_next;
		foundCommands.size += 1;
	}
}
int process_command(int number_of_arguments, char** arguments) {
	...
	int count = number_of_arguments - 1;
	int command_found = 0;
	...
	if (command_found) {
		if (count == commands[i].num_of_args) {
			...
			return i;
		} else if ((commands[i].num_of_args == -1) && (count > 0)) {
			...
			foundCommands.lh_first = &commands[i];
			foundCommands.size += 1;

			//	cprintf("%s : inccvalid number of args.\n",commands[i].name);
		...
		for (int j = 0; j < NUM_OF_COMMANDS; j++) {
			if (is_subseq(j, arguments[0])) {
				command_matched = 1;

				if (foundCommands.size == 0) {

					LIST_INIT(&foundCommands);
					found_list(&commands[j]);
					//cprintf("[+]  %s\n",foundCommands.lh_last);
		...
		if (command_matched) {
			...
			//cprintf("size after %d",foundCommands.size);
			return CMD_MATCHED;
		} else {
			return CMD_INVALID;
		}
