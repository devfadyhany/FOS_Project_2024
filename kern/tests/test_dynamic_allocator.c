...
uint32 allocSizes[numOfAllocs] = { 4 * kilo, 20 * sizeof(char) + sizeOfMetaData,
		1 * kilo, 3 * sizeof(int) + sizeOfMetaData, 2 * kilo,
		2 * sizeOfMetaData, 7 * kilo };
...
void test_initialize_dynamic_allocator() {
	...
	*(tmp_ptr + 1) = 1;
	...
	uint32* daEnd = (uint32*) (KERNEL_HEAP_START + initAllocatedSpace
			- sizeof(int));
	uint32* blkHeader = (uint32*) (KERNEL_HEAP_START + sizeof(int));
	uint32* blkFooter = (uint32*) (KERNEL_HEAP_START + initAllocatedSpace
			- 2 * sizeof(int));
	if (*daBeg != 1 || *daEnd != 1
			|| (*blkHeader != initAllocatedSpace - 2 * sizeof(int))
			|| (*blkFooter != initAllocatedSpace - 2 * sizeof(int))) {
		panic(
				"Content of header/footer and/or DA begin/end are not set correctly");
	}
...
#define numOfFFTests 3
	...
	uint32 expectedVAs[numOfFFTests] = { startVA + sizeOfMetaData / 2,
			startOf1st7KB, startVA + testSizes[0] + sizeOfMetaData / 2 };
	...
	//Check block
	expectedVA = (void*) (startVA + testSizes[0] + testSizes[2]
			+ sizeOfMetaData / 2);
	...
	*(tstEndVAs[numOfFFTests]) = 353;
	...
	cprintf(
			"	4: Try to allocate a block with a bit smaller size [internal fragmentation case]\n\n");
	is_correct = 1;
...
#define numOfBFTests 5
	...
	uint32 expectedVAs[numOfBFTests] = { startOf1st7KB, startOf1st2KB,
			startOf1st2KB + testSizes[1], startOf1st1KB, KERNEL_HEAP_START
					+ 2 * sizeof(int) };
...
void test_free_block_FF() {
	...
	if (check_block(va, expected_va, remainSize, 1) == 0)
		//			if(va != (curVA + sizeOfMetaData/2))
		panic(
				"test_free_block #2: WRONG ALLOC - alloc_block_FF return wrong address. Expected %x, Actual %x",
				expected_va, va);
	*(startVAs[idx]) = idx;
	*(midVAs[idx]) = idx;
	*(endVAs[idx]) = idx;
	...
	if (is_correct)
		is_correct = check_block(startVAs[numOfAllocs * allocCntPerSize],
	...
	va = alloc_block(actualSize, DA_FF);
	//Check returned va
	void* expected = (void*) (KERNEL_HEAP_START + sizeof(int)
			+ sizeOfMetaData / 2);
	...
	va = alloc_block(actualSize, DA_FF);
	//Check returned va
	expected = (void*) (KERNEL_HEAP_START + sizeof(int) + 5 * kilo
			+ sizeOfMetaData / 2);
	if (check_block(va, expected, expected_size, 1) == 0) {
		is_correct = 0;
...
void test_free_block_BF() {
	...
	if (check_block(va, expected_va, remainSize, 1) == 0)
		panic(
				"test_free_block #1: WRONG ALLOC - alloc_block_BF return wrong address.");
	*(startVAs[idx]) = idx;
	*(midVAs[idx]) = idx;
	*(endVAs[idx]) = idx;
	...
	for (int i = 0; i < idx; ++i) {
		if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i)
			panic(
					"test_free_block #2.%d: WRONG! content of the block is not correct. Expected %d",
					i, i);
	}
...
void test_realloc_block_FF() {
	...
	for (int i = 0; i < numOfAllocs; ++i) {
		for (int j = 0; j < allocCntPerSize; ++j) {
			...
			if (check_block(va, expectedVA, expectedSize, 1) == 0) {
				panic(
						"test_realloc_block_FF #1.1.%d: WRONG ALLOC - it return wrong address. Expected %x, Actual %x",
						idx, curVA + sizeOfMetaData, va);
			}
			curVA += allocSizes[i];
			*(startVAs[idx]) = idx;
	...
	*(endVAs[idx]) = idx;
	...
	for (int i = 0; i < idx; ++i) {
		if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i)
			panic(
					"test_realloc_block_FF #1.3.%d: WRONG! content of the block is not correct. Expected %d",
					i, i);
	}
	...
	for (int i = 0; i < idx; ++i) {
		...
		if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i)
			panic(
					"test_realloc_block_FF #2.4.%d: WRONG! content of the block is not correct. Expected %d",
					i, i);
	}
	...
	is_correct = 1;
	{
		...
		if (*(startVAs[blockIndex]) != blockIndex
			...
			is_correct = 0;
			cprintf(
					"test_realloc_block_FF #3.1.2: WRONG REALLOC! content of the block is not correct. Expected %d\n",
					blockIndex);
		}
	}
	if (is_correct) {
	...
	is_correct = 1;
	{
		...
		if (*(startVAs[blockIndex]) != blockIndex
			...
			is_correct = 0;
			cprintf(
					"test_realloc_block_FF #3.2.2: WRONG REALLOC! content of the block is not correct. Expected %d\n",
					blockIndex);
		}
	...
	is_correct = 1;
	{
		...
		struct BlockElement *newBlkAddr = (struct BlockElement *) (va + new_size
				+ 2 * sizeof(int));
		...
		if (*(startVAs[blockIndex]) != blockIndex
			...
			is_correct = 0;
			cprintf(
					"test_realloc_block_FF #4.1.3: WRONG REALLOC! content of the block is not correct. Expected %d\n",
					blockIndex);
		}
	...
	is_correct = 1;
	{
		...
		if (*(startVAs[blockIndex]) != blockIndex
			...
			is_correct = 0;
			cprintf(
					"test_realloc_block_FF #4.2.2: WRONG REALLOC! content of the block is not correct. Expected %d\n",
					blockIndex);
		}
	...
	cprintf(
			"[PARTIAL] test realloc_block with FIRST FIT completed. Evaluation = %d%\n",
			eval);
}
...
void test_realloc_block_FF_COMPLETE() {
	...
	*(endVAs[idx]) = idx;
	...
	//[3.3] reallocate in different place (relocate - split)
	cprintf("	3.3: reallocate in different place (relocate - split)\n\n");
	is_correct = 1;
	{
		blockIndex = 4 * allocCntPerSize - 1;
		new_size = allocSizes[3] + 2 * allocSizes[4] - sizeOfMetaData;
		expectedSize = ROUNDUP(new_size + sizeOfMetaData, 2);

		struct BlockElement* iterator;
		LIST_FOREACH(iterator, &freeBlocksList)
		{
			if (get_block_size(iterator) >= new_size) {
				expectedVA = iterator;
				break;
			}
		}
		...
		//check content of reallocated block
		int total_size = new_size + sizeOfMetaData;
		if (get_block_size(va) != total_size) {
			is_correct = 0;
			cprintf(
					"test_realloc_block_FF #3.3.2: WRONG REALLOC! content of the block is not correct. Expected %d\n",
					total_size);
		}
		...
		if (is_correct) {
			free_block(va);
			set_block_data(startVAs[blockIndex],
					get_block_size(startVAs[blockIndex]), 1);
			expectedNumOfFreeBlks--;
		}
	}

	//[3.4] reallocate in different place (relocate - No split)
	cprintf("	3.4: reallocate in different place (relocate - No split)\n\n");
	is_correct = 1;
	{
		blockIndex = 4 * allocCntPerSize - 1;
		...
		if (get_block_size(va) != total_size) {
			is_correct = 0;
			cprintf(
					"test_realloc_block_FF #3.4.2: WRONG REALLOC! content of the block is not correct. Expected %d\n",
					total_size);
		}
	...
	is_correct = 1;
	{
		...
		if (*(startVAs[blockIndex]) != blockIndex
			...
			is_correct = 0;
			cprintf(
					"test_realloc_block_FF #4.3.2: WRONG REALLOC! content of the block is not correct. Expected %d\n",
					blockIndex);
		}
	...
	cprintf(
			"[COMPLETE] test realloc_block with FIRST FIT completed. Evaluation = %d%\n",
			eval);
}
